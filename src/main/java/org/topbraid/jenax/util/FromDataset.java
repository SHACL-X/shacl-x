...
public class FromDataset extends DelegatingDataset {
    ...
    public FromDataset(Dataset delegate, Query query) throws GraphNotFoundException {
        super(delegate);
        defaultGraphs = new HashSet<>(query.getGraphURIs());
        namedGraphs = new HashSet<>(query.getNamedGraphURIs());
        initDefaultModel();
    }
    ...
    public boolean isEmpty() {
        return defaultModel.isEmpty() && namedGraphs.stream().map(this::getNamedModel).allMatch(Model::isEmpty);
    }
}
