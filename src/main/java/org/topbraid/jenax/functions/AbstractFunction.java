...
public abstract class AbstractFunction implements Function {
    ...
    public NodeValue exec(Binding binding, ExprList args, String uri, FunctionEnv env) {
        ...
        if (ExecStatisticsManager.get().isRecording() && ExecStatisticsManager.get().isRecordingNativeFunctions()) {
            ...
            sb.append(SSE.str(NodeFactory.createURI(uri), pm)).append("(");
            ...
            try {
                ...
                sb.append(" = ").append(FmtUtils.stringForNode(result.asNode(), pm));
            } catch (ExprEvalException ex) {
                sb.append(" : ").append(ex.getLocalizedMessage());
                throw ex;
            } finally {
                long endTime = System.currentTimeMillis();
                ExecStatistics stats = new ExecStatistics(sb.toString(), "(Native built-in function)", endTime - startTime, startTime, NodeFactory.createURI(uri));
