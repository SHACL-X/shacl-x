...
public class PyFactory {
    ...
    public static Node getNode(Object obj) {
        if (obj == null) {
            return null;
        } else if (obj instanceof PyTerm) {
            return ((PyTerm) obj).getNode();
    ...
    public static PyTerm asPyTerm(Node node) {
        if (node.isURI()) {
            return new PyNamedNode(node);
        } else if (node.isBlank()) {
            return new PyBlankNode(node);
        } else if (node.isLiteral()) {
            return new PyLiteral(node);
        } else {
            throw new IllegalArgumentException("Unsupported node type " + node);
        }
    }

    public static PyTriple asPyTriple(Triple triple) {
        return new PyTriple(triple);
    }
