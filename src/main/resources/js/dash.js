...
function validateClosed($value, $closed, $ignoredProperties, $currentShape) {
    ...
    let allowed = $shapes.query().match($currentShape, "sh:property", "?propertyShape").match("?propertyShape", "sh:path", "?path").filter(function (solution) {
    ...
    $data.query().match($value, "?predicate", "?object").filter(function (sol) {
        ...
        results.push({
            path: sol.predicate, value: sol.object
        });
    });
    return results;
}
...
function validateEqualsProperty($this, $path, $equals) {
    let results = [];
    let path = toRDFQueryPath($path);
    $data.query().path($this, path, "?value").forEach(function (solution) {
    ...
    $data.query().match($this, $equals, "?value").forEach(function (solution) {
...
let validateEqualsNode = function ($this, $equals) {
    let results = [];
    let solutions = 0;
    $data.query().path($this, $equals, "?value").forEach(function (solution) {
        solutions++;
...
function validateHasValueWithClass($this, $path, $hasValueWithClass) {
    return $data.query().path($this, toRDFQueryPath($path), "?value").filter(function (sol) {
        return new RDFQueryUtil($data).isInstanceOf(sol.value, $hasValueWithClass)
    }).hasSolution();
}
...
function validateLessThanProperty($this, $path, $lessThan) {
    let results = [];
    $data.query().path($this, toRDFQueryPath($path), "?value").match($this, $lessThan, "?otherValue").forEach(function (sol) {
...
function validateLessThanOrEqualsProperty($this, $path, $lessThanOrEquals) {
    let results = [];
    $data.query().path($this, toRDFQueryPath($path), "?value").match($this, $lessThanOrEquals, "?otherValue").forEach(function (sol) {
        let c = SHACL.compareNodes(sol.value, sol.otherValue);
        if (c == null || c > 0) {
            results.push({
                value: sol.value
...
function validateNodeKind($value, $nodeKind) {
    if ($value.isBlankNode()) {
        return T("sh:BlankNode").equals($nodeKind) || T("sh:BlankNodeOrIRI").equals($nodeKind) || T("sh:BlankNodeOrLiteral").equals($nodeKind);
    } else if ($value.isURI()) {
        return T("sh:IRI").equals($nodeKind) || T("sh:BlankNodeOrIRI").equals($nodeKind) || T("sh:IRIOrLiteral").equals($nodeKind);
    } else if ($value.isLiteral()) {
        return T("sh:Literal").equals($nodeKind) || T("sh:BlankNodeOrLiteral").equals($nodeKind) || T("sh:IRIOrLiteral").equals($nodeKind);
    }
}
...
function validateQualifiedHelper($this, $path, $qualifiedValueShape, $qualifiedValueShapesDisjoint, $currentShape) {
    ...
    return $data.query().path($this, toRDFQueryPath($path), "?value").filter(function (sol) {
        return SHACL.nodeConformsToShape(sol.value, $qualifiedValueShape) && !validateQualifiedConformsToASibling(sol.value, siblingShapes.toArray());
    }).getCount();
}
...
function validateSymmetric($this, $path, $symmetric) {
    ...
    if (T("true").equals($symmetric)) {
        let path = toRDFQueryPath($path);
        $data.query().path($this, path, "?value").filter(function (sol) {
...
function validateUniqueValueForClass($this, $uniqueValueForClass, $path) {
    let results = [];
    $data.query().path($this, toRDFQueryPath($path), "?value").path("?other", toRDFQueryPath($path), "?value").filter(function (sol) {
        return !$this.equals(sol.other);
    }).filter(function (sol) {
        return new RDFQueryUtil($data).isInstanceOf(sol.other, $uniqueValueForClass)
    }).forEach(function (sol) {
        results.push({
            other: sol.other, value: sol.value
        })
    });
    return results;
}
