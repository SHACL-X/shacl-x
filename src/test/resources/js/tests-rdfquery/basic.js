function bindNewLabel() {
    return RDFQuery($data).match("owl:Class", "rdfs:label", "?label").bind("?newLabel", function (sol) {
        return TermFactory.literal(sol.label.getValue(), "en-US")
    }).getNode("?newLabel");
}

function countAllSubClasses() {
    let count = RDFQuery($data).match("?subClass", "rdfs:subClassOf", "sh:Shape").getCount();
    return TermFactory.literal(count, T("xsd:decimal"));
}

function getFirstType() {
    return RDFQuery($data).match("sh:Shape", "rdf:type", "?type").getNode("?type");
}

function joinTwoBGPsThenFilter() {
    return RDFQuery($data).match("owl:Class", "rdfs:label", "?label").match("?otherClass", "rdfs:label", "?label").filter(function (sol) {
        return !T("owl:Class").equals(sol.otherClass)
    }).getNode("?otherClass");
}

function limit10() {
    return RDFQuery($data).match(null, "rdfs:label", null).limit(10).getCount();
}

function orderByVarProperty() {
    let list = RDFQuery($data).match("?property", "rdfs:domain", "rdf:Statement").orderBy("?property").getNodeArray("?property");
